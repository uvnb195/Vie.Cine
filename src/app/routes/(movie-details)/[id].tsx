...
import { resetSchedules } from '@/src/redux/privateSlice'
import { fetchMovie } from '@/src/redux/publicAsyncActions'
import { resetMovieDetail, setLoading } from '@/src/redux/publicSlice'
...
const DetailScreen = () => {
    ...
    const { colors } = theme
    const movieId = useLocalSearchParams().id.toString()
    ...
    useEffect(() => {
        if (movieInfo.movie && movieInfo.movie.id.toString() == movieId) {
            scrollY.value = 0
    ...
    useEffect(() => {
        auth.currentUser?.getIdToken().then((token) => {
            dispatch(getMovieSchedule({ token, movieId: parseInt(movieId) }))
        })
